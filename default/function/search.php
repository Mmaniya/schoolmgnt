...
echo $sql = "SELECT * FROM `ugpreference` WHERE `stuname` >= '$stuname' && `staffname` >= '$staffname' && `experience` = '$experience' && `selectyear`='$selectyear' && `selectedu`='$selectedu' && `popularity`='$popularity'";
$result = $conn->query($sql);

// if($result){
//     $_SESSION['message']['success'] = "<center><h6 style='color:green'>Add New Class.!</h6><center>";
//         header("Location: ../stuview.php?myid=$staffid");
// }
?>